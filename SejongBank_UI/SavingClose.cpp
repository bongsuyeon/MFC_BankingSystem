...
IMPLEMENT_DYNAMIC(SavingClose, CDialog)

SavingClose::SavingClose(CWnd* pParent /*=nullptr*/)
	: CDialog(IDD_SavingClose, pParent)
{
...
void SavingClose::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(SavingClose, CDialog)
	ON_CBN_SELCHANGE(IDC_COMBO1, &SavingClose::OnCbnSelchangeCombo1)
	ON_BN_CLICKED(IDOK, &SavingClose::OnBnClickedOk)
	ON_WM_ERASEBKGND()
	ON_WM_CTLCOLOR()
END_MESSAGE_MAP()
...
void SavingClose::OnCbnSelchangeCombo1()
{
...
void SavingClose::OnBnClickedOk()
{
	char pTmp[256];
	int IDLength;
...
BOOL SavingClose::OnInitDialog()
{
	...
	abc->SendInfo(SOC_Saving_Delete, pTmp);
...
BOOL SavingClose::OnEraseBkgnd(CDC* pDC)
{
...
HBRUSH SavingClose::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
